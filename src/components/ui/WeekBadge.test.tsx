...
describe('WeekBadge', () => {
  describe('Compact Mode (default)', () => {
    ...
    it('displays correct week number for each week', () => {
      ...
      weeks.forEach((week) => {
        const { unmount } = render(<WeekBadge week={week} />);
    ...
    it('calls onClick when clicked', () => {
      ...
      render(<WeekBadge week={0} onClick={handleClick} />);
    ...
    it('applies custom className', () => {
      render(<WeekBadge week={0} className="custom-class" data-testid="badge" />);
  ...
  describe('Detailed Mode (showDetails=true)', () => {
    it('renders detailed view with week info', () => {
      render(<WeekBadge week={0} showDetails />);
      expect(screen.getByText('Week 1')).toBeInTheDocument();
      expect(screen.getByText('Baseline')).toBeInTheDocument();
    ...
    it('shows weight adjustment', () => {
      render(<WeekBadge week={1} showDetails />);
    ...
    it('shows "Tap to change week" when onClick is provided', () => {
      const handleClick = vi.fn();
      render(<WeekBadge week={0} showDetails onClick={handleClick} />);
